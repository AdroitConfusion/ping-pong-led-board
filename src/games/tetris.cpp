...
namespace Tetris {
    ...
    void tetris(const int &joy_x, const int &joy_y, const bool &butt_z, const bool &butt_c, bool &new_game) {
        ...
        tetrisCheckController(joy_x, joy_y, butt_z);
        ...
        if (millis() - tetris_last_delay >= tetris_game_delay) {
            if (!tetrisGameOver) {
                ...
                moveTetronimo();
                tetris_dir = 0;
                if (tetrisFrameCount % 2 == 0 && tetris_rotate) {
                    rotateShape();
                    delay(20);
                    tetris_rotate = false;
                }
    ...
    void tetrisCheckController(const int &joy_x, const int &joy_y, const bool &butt_z) {
        if (joy_x < 50)
            tetris_dir = -1;
        else if (joy_x > 200)
            tetris_dir = 1;
            ...
            for (int i = gameWidthLeft; i <= gameWidthRight; i++) {
                ...
                // If i reaches the end, then that means there is a full row
                    ...
                    for (int l = gameHeightBottom; l > 0; l--)
                        if (l < j)
                            for (int m = 0; m <= gameWidthRight; m++)
                                savedBlocks[m][l + 1] = savedBlocks[m][l];
                }
            }
        }
    }
